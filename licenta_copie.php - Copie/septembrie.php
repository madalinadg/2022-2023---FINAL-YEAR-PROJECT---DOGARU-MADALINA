...
if(isset($_POST["import"])){
    ...
    if($_FILES["file"]["size"] > 0){
        ...
        while(($column = fgetcsv($file, 10000, ",")) !== FALSE){
            ...
            $sqlCheck = "SELECT COUNT(*) as count FROM septembrie WHERE eveniment_sep = '" . $column[0] . "' AND clicks_sep  = '" . $column[1] . "' AND pagini_sep  = '" . $column[2] . "' AND durata_sep  = '" . $column[3] . "' AND dispozitiv_sep  = '" . $column[4] . "' AND rezolutie_sep  = '" . $column[5] . "' AND locatie_sep  = '" . $column[6] . "'";
            ...
            if ($rowCheck['count'] == 0) {
                ...
                $sqlInsert = "Insert into septembrie ( eveniment_sep , clicks_sep , pagini_sep , durata_sep , dispozitiv_sep , rezolutie_sep , locatie_sep ) values ('" . $column[0] . "', '" . $column[1] . "', '" . $column[2] . "', '" . $column[3] . "', '" . $column[4] . "', '" . $column[5] . "', '" . $column[6] . "')";
...
<body id="septembrie">>
    ...
    <!-- main -->
        <main>
            <h1 class="title">Timpul pe site al utilizatorilor în luna septembrie</h1>
     ...
     <!-- content -->
        ...
        <?php
    // display upload data
        $sqlSelect = "SELECT * from septembrie";
        ...
        if(mysqli_num_rows($result)>0){
            ...
            <table>
                ...
                <tbody>
                    <tr>
                        <td class="narrow"><?php echo $row['nr_util_sep'];?></td>
                        <td class="narrow"><?php echo $row['eveniment_sep'];?></td>
                        <td class="narrow"><?php echo $row['clicks_sep'];?></td>
                        <td class="narrow"><?php echo $row['pagini_sep'];?></td>
                        <td class="narrow"><?php echo $row['durata_sep'];?></td>
                        <td class="narrow"><?php echo $row['dispozitiv_sep'];?></td>
                        <td class="narrow"><?php echo $row['rezolutie_sep'];?></td>
                        <td class="narrow"><?php echo $row['locatie_sep'];?></td>
                    </tr>
                </tbody>
        ...
        ?>
            ...
            <div id="custom-text-section">
                <h2>Interpretarea rezultatelor - Septembrie</h2>
                <textarea id="custom-text-septembrie" rows="4" cols="50"></textarea>
                <div class="btn-txt"> 
