...
if(isset($_POST["import"])){
    ...
    if($_FILES["file"]["size"] > 0){
        ...
        while(($column = fgetcsv($file, 10000, ",")) !== FALSE){
            ...
            $sqlCheck = "SELECT COUNT(*) as count FROM februarie WHERE eveniment_feb = '" . $column[0] . "' AND clicks_feb  = '" . $column[1] . "' AND pagini_feb  = '" . $column[2] . "' AND durata_feb  = '" . $column[3] . "' AND dispozitiv_feb  = '" . $column[4] . "' AND rezolutie_feb  = '" . $column[5] . "' AND locatie_feb  = '" . $column[6] . "'";
            ...
            if ($rowCheck['count'] == 0) {
                ...
                $sqlInsert = "Insert into februarie ( eveniment_feb , clicks_feb , pagini_feb , durata_feb , dispozitiv_feb , rezolutie_feb , locatie_feb ) values ('" . $column[0] . "', '" . $column[1] . "', '" . $column[2] . "', '" . $column[3] . "', '" . $column[4] . "', '" . $column[5] . "', '" . $column[6] . "')";
...
<body id="februarie">>
    ...
    <!-- main -->
        <main>
            <h1 class="title">Timpul pe site al utilizatorilor în luna februarie</h1>
     ...
     <!-- content -->
        ...
        <?php
    // display upload data
        $sqlSelect = "SELECT * from februarie";
        ...
        if(mysqli_num_rows($result)>0){
            ...
            <table>
                ...
                <tbody>
                    <tr>
                        <td class="narrow"><?php echo $row['nr_util_feb'];?></td>
                        <td class="narrow"><?php echo $row['eveniment_feb'];?></td>
                        <td class="narrow"><?php echo $row['clicks_feb'];?></td>
                        <td class="narrow"><?php echo $row['pagini_feb'];?></td>
                        <td class="narrow"><?php echo $row['durata_feb'];?></td>
                        <td class="narrow"><?php echo $row['dispozitiv_feb'];?></td>
                        <td class="narrow"><?php echo $row['rezolutie_feb'];?></td>
                        <td class="narrow"><?php echo $row['locatie_feb'];?></td>
                    </tr>
                </tbody>
        ...
        ?>
            ...
            <div id="custom-text-section">
                <h2>Interpretarea rezultatelor - Februarie</h2>
                <textarea id="custom-text-februarie" rows="4" cols="50"></textarea>
                <div class="btn-txt"> 
